const polinaJson = {
	"polina": {
        "title": {
            "en": "Polina Lappo",
            "ru": "Полина Лаппо"
        },
		"nav": {
            "about": {
                "en": "About",
                "ru": "О себе"
            },
            "works": {
                "en": "Works",
                "ru": "Работы"
            },
            "contacts": {
                "en": "Contacts",
                "ru": "Контакты"
            }
        },
        "header": {
            "about": {
                "en": "About",
                "ru": "О себе"
            },
            "works": {
                "en": "Works",
                "ru": "Работы"
            },
            "contacts": {
                "en": "Contacts",
                "ru": "Контакты"
            }
        },
        "vacancy": {
            "en": "JavaScript Developer",
            "ru": "JavaScript разработчик"
        },
        "info": {
            "en": "<p>Hi! I`m Polina!</p>"+
            "<p>I`m developing sites, landing pages and different web-applications. You can see my projects in section <span class='link' id='spanlink'>Work</span>.</p>"+
            "<p>If you have a cool idea of the site or application and You want to implement it, write to me on social networks or fill out the form on the site!</p>",
            "ru": "<p>Привет, я Полина!</p>"+
            "<p>Я занимаюсь разработкой сайтов, посадочных страниц и различных web-приложений. Вы можете посмотеть примеры в разделе <span class='link' id='spanlink'>Работы</span>.</p>"+
            "<p>Если у вас есть крутая идея сайта или приложения и Вы хотите реализовать ее, пишите мне в социальных сетях или заполните форму на сайте! </p>"
        },
        "btn":{
            "hire": {
                "en": "Write me",
                "ru": "Связаться со мной"
            },
            "download": {
                "en": "Get CV",
                "ru": "Скачать резюме"
            },
            "submit": {
                "en": "Send",
                "ru": "Отправить"
            },
            "resume":{
                "en": "resource/resume_lappo_en.pdf",
                "ru": "resource/resume_lappo_ru.pdf"
            }
        },
        "form": {
            "sotial": {
                "en": "",
                "ru": ""
            },
            "description": {
                "en": "Also, You can send a request for the developing of the project by filling out the form below.",
                "ru": "A еще Вы можете написать мне сообщение, заполнив форму ниже."
            },
            "aboutProject": {
                "en": "About Your Project",
                "ru": "Сообщение"
            },
            "aboutYou": {
                "en": "About You",
                "ru": "О себе"
            },
            "placeholder": {
                "project": {
                    "en": "Project type, goals, timeline, estimate etc",
                    "ru": "Тип проекта, цели, сроки, оценки и т.д."
                },
                "name": {
                    "en": "Name *",
                    "ru": "Имя *"
                },
                "email": {
                    "en": "Email *",
                    "ru": "Email *"
                },
                "phone": {
                    "en": "Phone",
                    "ru": "Телефон"
                }

            },
            "error": {
                "en": "Something is wrong. Try to send the message later or write me on social networks.",
                "ru": "Что-то пошло не так. Попробуйте отправить сообщение позже или напишите мне в социальных сетях."
            },
            "success": {
                "en": "Thank you! Your message has been sent successfully, I will reply You soon.",
                "ru": "Спасибо! Ваше сообщение успешно отправлено, я отвечу Вам в ближайшее время"
            },
            "homelink": {
                "en": "To home",
                "ru": "На главную"
            }
        }
	}
};